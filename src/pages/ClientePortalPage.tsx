/**
 * PORTAL CLIENTE — Vista unificada para clientes finales
 * Incluye aprobación de pre-facturas y consulta de trámites/ahorros fiscales
 */

import { useState } from 'react';
import {
  CheckCircle2, XCircle, Shield, FileText, Sparkles,
  Receipt, Lock, ExternalLink, AlertTriangle, LayoutDashboard,
} from 'lucide-react';
...
import { StellaClientPortal } from '@/components/zenith/StellaClientPortal';
...
export default function ClientePortalPage() {
  ...
  const handleAprobar = () => {
    ...
    toast.success('✅ Pre-factura aprobada. Registro de autorización completado.');
  };
  ...
  const handleRechazar = () => {
    ...
    toast.info('Rechazo registrado. El equipo operativo será notificado.');
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Portal de Cliente</p>
              </div>
            </div>
        ...
        <Tabs defaultValue="dashboard" className="space-y-6">
          <TabsList className="bg-muted/30 border border-border/50">
            <TabsTrigger value="dashboard" className="gap-1.5 text-xs">
              <LayoutDashboard className="w-3.5 h-3.5" /> Mis Trámites
            </TabsTrigger>
            <TabsTrigger value="invoice" className="gap-1.5 text-xs">
              <Receipt className="w-3.5 h-3.5" /> Aprobación de Factura
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="invoice" className="space-y-6 max-w-3xl">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-xl font-bold font-display text-foreground">Pre-Factura de Servicios Aduaneros</h1>
                <p className="text-sm text-muted-foreground mt-1">{preFactura.docNum} — MAWB {preFactura.mawb}</p>
              </div>
                ...
                <CardContent className="pt-4 pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Su aprobación ha sido registrada. El corredor procederá con la facturación formal.</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card className="glass-panel">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-display text-foreground tracking-wide">Datos del Despacho</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="glass-panel">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-display text-foreground tracking-wide">Desglose de Servicios</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                {reembolsables.length > 0 && (
                  <>
                    ...
                    <p className="text-xs text-amber-400 font-semibold uppercase tracking-wider">Gastos Reembolsables</p>
            ...
            <Card className="glass-panel">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-display text-foreground tracking-wide flex items-center gap-2">
                  <Receipt className="w-4 h-4 text-primary" /> Comprobantes de Terceros
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {preFactura.soportesTerceros.map((soporte) => (
                    <div key={soporte.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border/50">
                    ...
                    <Dialog open={showAprobacion} onOpenChange={setShowAprobacion}>
                      <DialogTrigger asChild>
                        <Button className="gap-1.5 bg-green-600 hover:bg-green-700 flex-1">
                          <CheckCircle2 className="w-4 h-4" /> Aprobar Facturación
                        </Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle className="flex items-center gap-2 font-display">
                            <CheckCircle2 className="w-5 h-5 text-green-400" /> Confirmar Aprobación
                          </DialogTitle>
                          <DialogDescription>
                            Al aprobar, confirma que ha revisado y acepta los montos. Se registrará la fecha, hora e IP de aprobación.
                          </DialogDescription>
                        </DialogHeader>
                    ...
                    <Dialog open={showRechazo} onOpenChange={setShowRechazo}>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          ...
                          <DialogDescription>Indique qué debe corregirse.</DialogDescription>
                        </DialogHeader>
                        <Textarea
                        ...
                        <DialogFooter>
                          ...
                          <Button variant="destructive" className="gap-1.5" onClick={handleRechazar}>
                            <XCircle className="w-4 h-4" /> Enviar Rechazo
                          </Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
            ...
            <div className="text-center text-xs text-muted-foreground/50 space-y-1 py-4">
              <div className="flex items-center justify-center gap-1">
                ...
                <span>Protegido por Motor de Integridad</span>
              </div>
              {preFactura.zodHashIntegridad && (
                <p className="font-mono text-[10px]">Hash: {preFactura.zodHashIntegridad.slice(0, 24)}...</p>
              )}
